...
fn inspect(
    ...
    let mut notes = conn.prepare("SELECT pool, id_within_pool_table FROM v_received_outputs WHERE account_id = ?1 AND pool != 0")?;
    let notes = notes
        .query_map([account], |r| {
...
mod tests {
    ...
    fn absence_is_not_spend_evidence() {
        ...
        c.execute_batch("INSERT INTO v_received_outputs VALUES(1,3,1)")
            .unwrap();
        assert!(!inspect(&c, &[1], "").unwrap().all_funds_spent_elsewhere);
    }
    #[test]
    fn competing_spend_needs_six_confirmations_and_conflicts_with_our_input() {
        let c = db();
        c.execute_batch("INSERT INTO v_received_outputs VALUES(1,3,1); INSERT INTO transactions(id_tx, txid, mined_height, expiry_height) VALUES(1,X'AA',NULL,140),(2,X'BB',101,140); INSERT INTO sent_notes VALUES(1,1),(1,2); UPDATE transactions SET created=NULL WHERE id_tx=2; INSERT INTO v_received_output_spends VALUES(3,1,1),(3,1,2)").unwrap();
        ...
        c.execute_batch("INSERT INTO v_received_outputs VALUES(1,3,2)")
            .unwrap();
        assert!(!inspect(&c, &[1], "aa").unwrap().all_funds_spent_elsewhere);
    }
    #[test]
    ...
    fn own_receipt_and_reorg_are_not_losses() {
        let c = db();
        c.execute_batch("INSERT INTO v_received_outputs VALUES(1,3,1); INSERT INTO transactions(id_tx, txid, mined_height, expiry_height) VALUES(1,X'AA',100,140); INSERT INTO sent_notes VALUES(1,1); INSERT INTO v_received_output_spends VALUES(3,1,1)").unwrap();
        assert!(!inspect(&c, &[1], "").unwrap().all_funds_spent_elsewhere);
